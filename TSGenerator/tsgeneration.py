...
class TSGeneration:
    ...
    def print_info(self):
        ...
        print('{0:<32}{1:<6}: {2:.2g}'.format('Timing interval'           , '[s]' , self.dt))
        print('{0:<32}{1:<6}: {2}'    .format('Number of bins per segment', ''    , self.n_bin))
        print('{0:<32}{1:<6}: {2}'    .format('Total duration'            , '[s]' , self.c_t))
        print('{0:<32}{1:<6}: {2:.2g}'.format('Minimum frequency'         , '[Hz]', self.f_min))
        print('{0:<32}{1:<6}: {2:.2g}'.format('Maximum frequency'         , '[Hz]', self.f_max))
        print('{0:<32}{1:<6}: {2}'    .format('Number of segments'        , ''    , self.n_seg))
        print('{0:<32}{1:<6}: {2}'    .format('Output filename'           , ''    , self.name_outfile))
        print('')
    ...
    def add_average(self, mu):
        self.mu=mu
        self.xss=self.xss+self.mu
    ...
    def write_data(self):
        for i_seg in range(self.n_seg):
            ...
            hdu_ext.header['MU']  =(self.mu   , 'Average value')
            if i_seg==0:
